  ...
  if (Serial1.available() > 0) {
    ...
    if ((UDPBuffer[0] == '/') && (UDPBuffer[2] == '/') && ((UDPBuffer[1] == '1') || (UDPBuffer[1] == '2'))) {
...
#ifdef OSCDEBUG2
        SerialUSB.print("$FAD1");
        SerialUSB.print(OSCcommandType);
        SerialUSB.print("$");
#endif
        return;
      } // end fadder
      ...
      if ((UDPBuffer[3] == 'u') && (UDPBuffer[2] == 's') && (UDPBuffer[1] == 'h')) {
        ...
        //SerialUSB.println(commandType);
#ifdef OSCDEBUG2
        SerialUSB.print("$P");
        SerialUSB.print(UDPBuffer[0] - '1');
        SerialUSB.print(":");
#endif
          return;
        } // end push
        ...
        switch (OSCcommandType) {
          case 1:
            ...
            if ((OSCtouchMessage) && (value == 0)) {
              OSCfader[0] = 0.5;
              //SerialUSB.println("TOUCH_X");
              OSC_MsgSend("/1/fader1\0\0\0,f\0\0\0\0\0\0", 20, 0.5);
            }
#ifdef OSCDEBUG
            SerialUSB.print("$F1:");
            SerialUSB.println(OSCfader[0]);
#endif
            break;
          case 2:
            value = OSC_extractParamFloat(0);
            ...
            if ((OSCtouchMessage) && (value == 0)) {
              OSCfader[1] = 0.5;
              //SerialUSB.println("TOUCH_Y");
              OSC_MsgSend("/1/fader2\0\0\0,f\0\0\0\0\0\0", 20, 0.5);
            }
#ifdef OSCDEBUG
            SerialUSB.print("$F2:");
            SerialUSB.println(OSCfader[1]);
#endif
            break;
          case 3:
...
#ifdef OSCDEBUG
            SerialUSB.print("$XY1:");
            SerialUSB.print(OSCxy1_x);
            SerialUSB.print(",");
            SerialUSB.println(OSCxy1_y);
#endif
            break;
          case 12:
...
#ifdef OSCDEBUG
            SerialUSB.print("$XY2:");
            SerialUSB.print(OSCxy2_x);
            SerialUSB.print(",");
            SerialUSB.println(OSCxy2_y);
#endif
            break;
          case 40:
            // MOVE
